		...
		this.thenOpen(link + '?setDevice=desktop',function(){
			...
			if(this.exists('#prod_title')){
				...
				var comments = this.evaluate(function(){
					//$('#reviewslist ul.ratRev-PagingList li.ui-listItem').last().find('a').click();
					var reviews = [];
				...
				var save_record = this.evaluate(function(record){
					/*$.ajax({
					  type: "POST",
					  url: "http://localhost/crawler/index.php/crawler/save",
					  data: record,
					  success: function(){},
					  dataType: "json",
					  contentType : "application/json"
					});*/
					var wsurl = 'http://localhost/crawler/index.php/crawler/save'; 
					// return __utils__.sendAJAX(wsurl, "POST" , {data : JSON.stringify(record)}, false, { contentType: "application/json" });
					return __utils__.sendAJAX(wsurl, 'POST',null);
				},record);

				// utils.dump(record);
				//write record to text file
				this.saveJSON = function(what) {
				    // require('fs').write('export/data.json', JSON.stringify(what, null, '  '), 'w');
				...
				//call php save data
				/*this.open('http://localhost/crawler/index.php/crawler/save',function(){
